...
current_X = vertcat(start_x,start_y,starting_angle,start_vx,start_vy,start_w)
...
ocp.set_der(x, (vx*cos(theta) - vy*sin(theta)))
ocp.set_der(y, (vx*sin(theta) + vy*cos(theta)))
ocp.set_der(theta, w)
ocp.set_der(vx, vx_dot)
ocp.set_der(vy, vy_dot)
ocp.set_der(w, w_dot)
...
ocp.subject_to( (-max_linear_acc <= vx_dot) <= max_linear_acc )
ocp.subject_to( (-max_linear_acc <= vy_dot) <= max_linear_acc )
ocp.subject_to( (-max_rot_acc <= w_dot) <= max_rot_acc )
ocp.subject_to( (lower_speed <= vx) <= upper_speed )
ocp.subject_to( (lower_speed <= vy) <= upper_speed )
ocp.subject_to( (-max_rot_speed <= w) <= max_rot_speed )

X = vertcat(x, y, theta, vx, vy, w)
