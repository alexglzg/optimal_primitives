...
class TrajectoryGenerator:
    ...
    def _create_path(
        self, x2: float, y2: float, end_angle: float, start_angle: float
    ) -> Path:
        """
        ...
        end_angle_normalized = normalize_angle(end_angle)
        start_angle_normalized = normalize_angle(start_angle)

        if (abs(start_angle - end_angle) > abs(start_angle_normalized - end_angle_normalized)):
            start_angle = start_angle_normalized
            end_angle = end_angle_normalized
